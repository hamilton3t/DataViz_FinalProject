...
## select winner team, output is average age of team, compared to age of winner
## once team is selected, group by team, and summarise mean age, also display winner age
## make graph, plot, etc.. nice
```
...
Ideas:
-***winner age vs team age, in shiny, select year, then select team, output is...
...
server <- function(input, output, session) {
 ...
 #   observeEvent(input$team_sel, {
  ...
  #  year_choices <- winner_join |> filter(team == input$team_sel) |>
  #    distinct(winner_team) |> pull(winner_team)
    
  #  updateSelectInput(inputId = "year_sel",
 ...
 # })
  ...
  output$age_plot <- renderPlot({
   ...
   # ggplot(data = age_reactive(), aes(x = winner_team, y = team_mean_age)) +
    #  geom_col() +
   #   theme_minimal()
    
    ggplot(age_reactive(), aes(x = winner_name)) +
      geom_bar(aes(y = winner_mean_age, fill = "Winner"), stat = "identity", 
               position = "dodge") +
      geom_bar(aes(y = team_mean_age, fill = "Team"), stat = "identity", position = "dodge") +
      theme_minimal()
  })
}
